...
for i in range(0, len(resultFiles), 3):
    ...
    for y in range(3):
        ax[y].plot(timeS[:len(voltages[y])], voltages[y], label=labels[y], color='#008585',zorder=20)
        ax[y].set(ylabel='Voltage [V]')
        ax[y].legend(loc="lower right")
        ax[y].margins(x=0)
        ax[y].axhline(3.0, color='grey', ls='--',zorder=10)
        ax[y].axhline(2.7, color='#74a892', ls='--',zorder=10)
        if y != 0:
            ax[y].axhline(1.9, color='#d68a58', ls='--',zorder=10)
        ax[y].axhline(1.7, color='#642915', ls='--',zorder=10)
        ax[y].set(ylim=[-.1, 3.5], yticks=[0,1,2,3])
    ax[3].plot(timeS, led, label='LED', color='#d68a58')
