...
int checkpoint_recover() {
    ...
    nrfx_gpiote_out_clear(GPIO_PIN);
    //return retrieve_ram_from_flash(); // FULL
    return 0; // SIMPLIFIED
}
...
void get_program_state(uint32_t * buf) { // Number of stored values needs to match CHECKPOINT_WORDS
    ...
    buf[0] = next_state; //17
    buf[1] = current_sample; //18

    uint16_t data_index = 2; //19
...
void set_program_state(uint32_t * buf) {
    ...
    next_state = buf[0]; //buf[17]; 
    current_sample = buf[1]; //buf[18]; // Could have to set current_sample to 0
    //next_state = 0;
    //current_sample = 0;
    uint16_t data_index = 2; //19;
    for (int i = 0; i < NUM_SAMPLES*SAMPLE_SIZE; ++i) {
